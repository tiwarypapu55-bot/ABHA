...
-- Ensure columns referenced in patient_timeline exist on existing tables before view creation
ALTER TABLE public.patient_vitals ADD COLUMN IF NOT EXISTS recorded_by UUID;
ALTER TABLE public.patient_vitals ADD COLUMN IF NOT EXISTS recorded_at TIMESTAMPTZ DEFAULT NOW();
ALTER TABLE public.clinical_notes ADD COLUMN IF NOT EXISTS author_id UUID;
ALTER TABLE public.test_requests ADD COLUMN IF NOT EXISTS requested_by UUID;
ALTER TABLE public.invoices ADD COLUMN IF NOT EXISTS issued_by UUID;
ALTER TABLE public.admissions ADD COLUMN IF NOT EXISTS doctor_id UUID;
...
-- Ensure all foreign key referenced columns exist on older tables
...
ALTER TABLE public.test_requests ADD COLUMN IF NOT EXISTS requested_by UUID;
ALTER TABLE public.pharmacy_purchases ADD COLUMN IF NOT EXISTS recorded_by UUID;
...
ALTER TABLE public.nursing_notes ADD COLUMN IF NOT EXISTS nurse_id UUID;
ALTER TABLE public.birth_records ADD COLUMN IF NOT EXISTS doctor_id UUID;
ALTER TABLE public.external_reports ADD COLUMN IF NOT EXISTS uploaded_by UUID;
ALTER TABLE public.radiology_records ADD COLUMN IF NOT EXISTS requested_by UUID;
ALTER TABLE public.maternity_deliveries ADD COLUMN IF NOT EXISTS surgeon_id UUID;
ALTER TABLE public.nursing_handovers ADD COLUMN IF NOT EXISTS outgoing_nurse_id UUID;
ALTER TABLE public.nursing_handovers ADD COLUMN IF NOT EXISTS incoming_nurse_id UUID;
...
CREATE TABLE IF NOT EXISTS public.patients_abdm_link (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    patient_id UUID REFERENCES public.patients(id) ON DELETE CASCADE,
    abha_number VARCHAR(17) UNIQUE NOT NULL,
    abha_address VARCHAR(100) UNIQUE NOT NULL,
    aadhaar_verification_status VARCHAR(20) DEFAULT 'VERIFIED',
...
CREATE TABLE IF NOT EXISTS public.hpr_practitioners (
    ...
    hpr_id VARCHAR(100) UNIQUE NOT NULL,
    qualification TEXT NOT NULL,
    registration_number VARCHAR(50) UNIQUE NOT NULL,
    specialization TEXT NOT NULL,
    status VARCHAR(20) DEFAULT 'Active_HPR',
...
CREATE TABLE IF NOT EXISTS public.pmjay_claims (
    ...
    sachis_reconciliation_status VARCHAR(20) DEFAULT 'Pending',
...
ALTER TABLE public.hfr_facility_sync ENABLE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS "Enable read for all authenticated users" ON public.patients_abdm_link;
CREATE POLICY "Enable read for all authenticated users"
ON public.patients_abdm_link FOR SELECT
TO authenticated, anon
USING (true);

DROP POLICY IF EXISTS "Enable write for receptionists and admins" ON public.patients_abdm_link;
...
-- 2. Consents
DROP POLICY IF EXISTS "Allow read consents to clinician on record" ON public.abdm_consent_requests;
CREATE POLICY "Allow read consents to clinician on record"
ON public.abdm_consent_requests FOR SELECT
TO authenticated, anon
USING (true);

DROP POLICY IF EXISTS "Allow write consents to authorized clinicians" ON public.abdm_consent_requests;
...
DROP POLICY IF EXISTS "Allow read claims to billing operators" ON public.pmjay_claims;
CREATE POLICY "Allow read claims to billing operators"
ON public.pmjay_claims FOR SELECT
TO authenticated, anon
USING (true);

DROP POLICY IF EXISTS "Allow write claims to authorized billing/admins" ON public.pmjay_claims;
...
-- 4. Audit logger
DROP POLICY IF EXISTS "Compliance audit trail read-only for auditors" ON public.abdm_compliance_audit_logs;
CREATE POLICY "Compliance audit trail read-only for auditors"
ON public.abdm_compliance_audit_logs FOR SELECT
TO authenticated, anon
USING (true);

DROP POLICY IF EXISTS "Allow system log insertions" ON public.abdm_compliance_audit_logs;
CREATE POLICY "Allow system log insertions"
ON public.abdm_compliance_audit_logs FOR INSERT
TO authenticated, anon
WITH CHECK (coalesce(auth.role(), '') IS NOT NULL);

-- Create policies for remaining tables so client can call them freely
DROP POLICY IF EXISTS "Enable select for hpr" ON public.hpr_practitioners;
CREATE POLICY "Enable select for hpr" ON public.hpr_practitioners FOR SELECT TO authenticated, anon USING (true);
DROP POLICY IF EXISTS "Enable write for hpr" ON public.hpr_practitioners;
CREATE POLICY "Enable write for hpr" ON public.hpr_practitioners FOR ALL TO authenticated, anon USING (coalesce(auth.role(), '') IS NOT NULL) WITH CHECK (coalesce(auth.role(), '') IS NOT NULL);

DROP POLICY IF EXISTS "Enable select for hfr" ON public.hfr_facility_sync;
CREATE POLICY "Enable select for hfr" ON public.hfr_facility_sync FOR SELECT TO authenticated, anon USING (true);
DROP POLICY IF EXISTS "Enable write for hfr" ON public.hfr_facility_sync;
CREATE POLICY "Enable write for hfr" ON public.hfr_facility_sync FOR ALL TO authenticated, anon USING (coalesce(auth.role(), '') IS NOT NULL) WITH CHECK (coalesce(auth.role(), '') IS NOT NULL);
...
-- Revoke execute on public.rls_auto_enable if it is present to comply with security advisories
DO $$
BEGIN
  IF EXISTS (
